...
class Providers_twitter
{
    ...
    function getAccessToken($pass_value)
    {
        ...
        if (empty($_GET['oauth_token']) & empty($_GET['oauth_verifier'])) {
            ...
            $oauth_callback = getSiteUri();
            $result = $twitter->getRequestToken($oauth_callback);
            $redirect_to = $twitter->getAuthorizeURL($result);
            header("Location: " . $redirect_to);
        } else {
            $twitter = new TwitterOAuth($pass_value['api_key'], $pass_value['secret_key'], $_GET['oauth_token'], $_GET['oauth_verifier']);
