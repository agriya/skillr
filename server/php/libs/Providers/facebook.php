...
class Providers_facebook
{
    function getAccessToken($pass_value)
    {
        ...
        $_response = explode('&', $_response);
        $access_token = str_replace('access_token=', '', $_response[0]);
        return $access_token;
    }
    function getUserProfile($access_token, $provider_details = '')
    {
        ...
        try {
            $graphApiUrl = 'https://graph.facebook.com/me?access_token=' . $access_token . '&fields=id,name,first_name,last_name,link,website,gender,bio,email,hometown,birthday';
